...
def calcularDiferenciaSimetrica(conjuntos):
    ...
    # Verificar que haya al menos dos conjuntos
    if len(conjuntos) < 2:
        resultado = conjuntos
    else:
        repetidos = []
        
        # Asegurarse de que los elementos sean conjuntos
        conjuntos[0] = set(conjuntos[0])
        conjuntos[1] = set(conjuntos[1])
        
        union = conjuntos[0] | conjuntos[1]  # Usar '|' para la unión de conjuntos
        
        # Encontrar los elementos repetidos en ambos conjuntos
        for element in conjuntos[0]:
            if element in conjuntos[1] and element not in repetidos:
                repetidos.append(element)
        
        # Generar la nueva lista excluyendo los repetidos
        conjuntos[1] = [element for element in union if element not in repetidos]
        
        # Eliminar el primer conjunto y continuar recursivamente
        del conjuntos[0]
        resultado = calcularDiferenciaSimetrica(conjuntos)
...
if conjuntos:
    ...
    print("Unión de los conjuntos:", union_conjuntos(*conjuntos))

    if len(conjuntos) > 1:
        conjunto_base = conjuntos[0]
        otros_conjuntos = conjuntos[1:]
        print("Diferencia de los conjuntos:", diferencia_conjuntos(conjunto_base, *otros_conjuntos))
        print("Intersección de los conjuntos:", calcularInterseccion(*conjuntos))
        print("Diferencia simétrica de los conjuntos:", calcularDiferenciaSimetrica(conjuntos))
